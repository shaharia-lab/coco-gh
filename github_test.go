...
func (_m *GraphQLClientMock) Query(ctx context.Context, q interface{}, variables map[string]interface{}) error {
...
func (_m *RESTClientMock) GetCommit(ctx context.Context, owner string, repo string, sha string, opts *github.ListOptions) (*github.RepositoryCommit, *github.Response, error) {
...
func (_m *RESTClientMock) ListCommits(ctx context.Context, owner string, repo string, opts *github.CommitsListOptions) ([]*github.RepositoryCommit, *github.Response, error) {
...
func TestGitHubClient_GetFilePathsForRepositories(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			restClient := new(RESTClientMock)

			// Create a GitHub client with the mock client and configuration
			client := NewGitHubClient(restClient, graphQLClient, tt.config)

			// Set up the mock to return the query object and populate Entries
			graphQLClient.On("Query", mock.Anything, mock.Anything, mock.Anything).Run(func(args mock.Arguments) {
...
func TestGitHubClient_GetChangedFilePathsSince(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ghClient := new(GraphQLClientMock)
			restClient := new(RESTClientMock)
			restClient.On("ListCommits", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return([]*github.RepositoryCommit{{
				SHA: github.String("1234567890"),
			}}, nil, nil)
			restClient.On("GetCommit", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(&github.RepositoryCommit{Files: tt.detectedChanges}, nil, nil)

			client := NewGitHubClient(restClient, ghClient, tt.config)
